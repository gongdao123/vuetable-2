...
export default {
  ...
  methods: {
    ...
    loadSuccess: function (response) {
      ...
      if (this.tablePagination === null) {
        this.warn('vuetable: pagination-path "' + this.paginationPath + '" not found. ' +
          'It looks like the data returned from the sever does not have pagination information ' + 'or you may have set it incorrectly.'
        )
      }
    ...
    callCallback: function (field, item) {
      ...
      let func = args.shift()
      let callback = this.$parent[func]
      if (typeof callback === 'function') {
        let value = this.getObjectValue(item, field.name)
        return (args.length > 0) ? callback(value, ...args) : callback(value)
      }
